import type { CreditType } from "../simulator/types";

export interface ChileanRateBenchmark {
  creditType: CreditType;
  institution: string;
  averageRate: number; // tasa anual en porcentaje
  updatedAt: string;
  source: string;
}

export const chileanRateBenchmarks: ChileanRateBenchmark[] = [
  {
...
export function findBenchmark(
  creditType: CreditType,
  institution: string,
): ChileanRateBenchmark | undefined {
  return chileanRateBenchmarks.find(
    (entry) => entry.creditType === creditType && entry.institution === institution,
  );
}
